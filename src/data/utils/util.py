...
def rotmat2quat_torch(R):
    ...
    q = _move_to_device(torch.zeros(R.shape[0], 4).float())
...
def normalization(data, parameters=None):
    ...
    if parameters is None:
        ...
        # For each dimension
        for i in range(dim):
            mean[i] = np.mean(norm_data[:, i])
            norm_data[:, i] = norm_data[:, i] - mean[i]
            var[i] = np.std(norm_data[:, i])
...
def renormalization(norm_data, norm_parameters):
    ...
    for i in range(dim):
        renorm_data[:, i] = renorm_data[:, i] * (std[i] + 1e-6)
        renorm_data[:, i] = renorm_data[:, i] + mean[i]
